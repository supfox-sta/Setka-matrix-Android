/*
 * Copyright (c) 2025 Element Creations Ltd.
 * Copyright 2024, 2025 New Vector Ltd.
 *
 * SPDX-License-Identifier: AGPL-3.0-only OR LicenseRef-Element-Commercial.
 * Please see LICENSE files in the repository root for full details.
 */

package io.element.android.features.call.impl.ui

import androidx.compose.ui.tooling.preview.PreviewParameterProvider
import io.element.android.libraries.architecture.AsyncData

open class CallScreenStateProvider : PreviewParameterProvider<CallScreenState> {
    override val values: Sequence<CallScreenState>
        get() = sequenceOf(
            aCallScreenState(),
            aCallScreenState(urlState = AsyncData.Loading()),
            aCallScreenState(urlState = AsyncData.Failure(Exception("An error occurred"))),
            aCallScreenState(webViewError = "Error details from WebView"),
        )
}

internal fun aCallScreenState(
    urlState: AsyncData<String> = AsyncData.Success("https://call.element.io/some-actual-call?with=parameters"),
    webViewError: String? = null,
    userAgent: String = "",
    isCallActive: Boolean = true,
    isInWidgetMode: Boolean = false,
    isAudioOnly: Boolean = false,
    isMicrophoneEnabled: Boolean = true,
    isVideoEnabled: Boolean = false,
    showJoinCallAction: Boolean = false,
    callConnectionState: CallConnectionState = CallConnectionState.Calling,
    callPeerId: String? = null,
    callPeerName: String? = null,
    callPeerAvatarUrl: String? = null,
    callAccentColorHex: String? = null,
    callTopBarBackgroundColorHex: String? = null,
    callTopBarTextColorHex: String? = null,
    callBlurEnabled: Boolean = true,
    callAnimationsEnabled: Boolean = true,
    callAudioBackgroundImageUrl: String? = null,
    callPreferEarpieceByDefault: Boolean = true,
    callProximitySensorEnabled: Boolean = true,
    eventSink: (CallScreenEvents) -> Unit = {},
): CallScreenState {
    return CallScreenState(
        urlState = urlState,
        webViewError = webViewError,
        userAgent = userAgent,
        isCallActive = isCallActive,
        isInWidgetMode = isInWidgetMode,
        isAudioOnly = isAudioOnly,
        isMicrophoneEnabled = isMicrophoneEnabled,
        isVideoEnabled = isVideoEnabled,
        showJoinCallAction = showJoinCallAction,
        callConnectionState = callConnectionState,
        callPeerId = callPeerId,
        callPeerName = callPeerName,
        callPeerAvatarUrl = callPeerAvatarUrl,
        callAccentColorHex = callAccentColorHex,
        callTopBarBackgroundColorHex = callTopBarBackgroundColorHex,
        callTopBarTextColorHex = callTopBarTextColorHex,
        callBlurEnabled = callBlurEnabled,
        callAnimationsEnabled = callAnimationsEnabled,
        callAudioBackgroundImageUrl = callAudioBackgroundImageUrl,
        callPreferEarpieceByDefault = callPreferEarpieceByDefault,
        callProximitySensorEnabled = callProximitySensorEnabled,
        eventSink = eventSink,
    )
}
